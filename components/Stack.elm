    ...
    StackCard gifAction ->
      case (List.head model) of
        Just gif ->
          let
            ( gif, effects ) =
              StackCard.update gifAction gif global
          in
            ...
            Just tail ->
              div
                ...
                (StackCard.view (Signal.forwardTo address StackCard) first
                  :: (List.reverse
                        (List.indexedMap
                          (\id gif -> Gif.stackView id gif.gif)
                          (List.take 5 tail)
                        )
                     )
                )
    ...
    div
      ...
      , div
          ...
          [ i
              ...
              , crossStyle
              , onClick address (StackCard StackCard.SwipeLeft)
              ]
              [ text "clear" ]
          , i
              [ class "material-icons hover-btn"
              , tickStyle
              , onClick address (StackCard StackCard.SwipeRight)
              ]
              [ text "favorite" ]
          ]
      ]
