...
loginSignal =
  Signal.filterMap (\auth -> case auth of
    Just auth ->
      Just (Login auth)
    Nothing ->
      Nothing) NoOp loginBox.signal
...
init loc =
  let effects = subscribeAuth (\auth -> Signal.send loginBox.address auth) loc
                  |> Task.toMaybe
                  |> Task.map (\_ -> NoOp)
                  |> Effects.task
  in
      ...
      case model of
        Nothing ->
          let user = (getUserFromAuth auth)
              ...
              effects = case user.subscription of
                Nothing ->
                  ElmFire.subscribe
      ...
      let effects = case model of
        Just user ->
          case user.subscription of
            Just sub ->
              let unsubscribeEffect = ElmFire.unsubscribe sub
                                        |> Task.toMaybe
                                        |> Task.map (always NoOp)
                                        |> Effects.task
                  logoutEffect = ElmFire.Auth.unauthenticate root
                                        |> Task.toMaybe
                                        |> Task.map (always NoOp)
                                        |> Effects.task
              in
...
loginView address model =
  ...
  in
    div [ containerStyle ] [ h1 [ titleStyle ] [ text "Gipher" ]
                            , div [ btnStyle, class "login-btn", onClick address LoginRequest ]
